...
import {setGeneratedText,setRegenerateText} from "../../../features/userSlice";
...
const History = (props) => {
    ...
    const handleClick = async (historyId,title) => {
        ...
        try {
            ...
            dispatch(setRegenerateText(title));
        } catch (error) {
            console.log(error);
        }
    }
            ...
            {histories.map(history => (
                <li key={history._id}>
                    <Link to="#" className={`flex items-center p-2 text-gray-900 rounded-lg dark:text-gray-500 group hover:bg-gray-700 hover:text-white relative ${activeHistory === history._id ? 'activeHistory' : ''}`}>
                        ...
                        <button onClick={() => handleClick(history._id,history.title)}>
